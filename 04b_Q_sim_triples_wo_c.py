...
# find similarity between question text and triples t26
# triples without coreference chains
...
import argparse
parser = argparse.ArgumentParser(description='q-t')
...
variation = args.var #'sent'
# ttfname = 'data/text_tokens_gen2oie.pkl' # 95 percentile of token length (text = q + p) is less than 512

text_tokens = []

for fi,fname in tqdm(enumerate(ml), ncols=80, total=len(ml), desc='Making text_tokens'):
    # print('\n',fname)
    ...
    if 'each' in variation:
        for l1 in oie_data['context_triples']:
            for l2 in l1:
                if 'gen2oie' in fname:
                    triples.append(l2)
                else:
        ...
        for l1 in oie_data['context_triples']:
            ...
            for l2 in l1:
                if 'gen2oie' in fname:
                    para_triples.append(' '.join(l2))
                else:
                    for l3 in l2:
                        para_triples.append(' '.join(l3))
            triples.append('. '.join(para_triples))
    if 'q_context_'+variation+'_triple_scores' in oie_data:
        if len(list(oie_data['q_context_'+variation+'_triple_scores'].values())[0][1:]) == len(triples):
            continue
    ...
    for qid in qids:
        ...
        try:
            ...
            continue
        for t in triples:
            text = question + ' ' + (' '.join(t) if type(t) == list else t)
            text_list.append(text)
            # text_tokens.append(len(tokenizer.tokenize(text)))
        result = model.predict_this(text_list).tolist()
        if 'q_context_'+variation+'_triple_scores' not in oie_data:
            oie_data['q_context_'+variation+'_triple_scores'] = {}
        oie_data['q_context_'+variation+'_triple_scores'][qid] = [question]+result
